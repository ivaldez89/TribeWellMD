...
// Day mode: green-700 for default, white for selected
// Dark mode: always white on gradient
const PlaylistIcon = ({ type, isPlaying, isSelected }: { type: string; isPlaying?: boolean; isSelected?: boolean }) => {
  // Icon glyph color: green-700 for day default, white for selected/dark
  const iconClass = isSelected
    ? 'text-white'
    : 'text-green-700 dark:text-white/90';

  if (isPlaying) {
    return <span className={`w-3 h-3 rounded-full animate-pulse ${isSelected ? 'bg-white' : 'bg-green-700 dark:bg-white/90'}`} />;
  }
  ...
  switch (type) {
    case 'focus':
      ...
      return (
        <svg className={`w-5 h-5 ${iconClass}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    ...
    case 'piano':
      // Piano keys pattern
      return (
        <svg className={`w-5 h-5 ${iconClass}`} viewBox="0 0 24 24" fill="currentColor">
    ...
    default:
      return <span className={`w-2 h-2 rounded-full ${isSelected ? 'bg-white/60' : 'bg-green-700 dark:bg-white/60'}`} />;
  }
};
                ...
                <div className="space-y-2">
                  {STUDY_PLAYLISTS.map(playlist => {
                    ...
                    return (
                      ...
                      >
                        {/* Icon container: day mode uses solid green, dark mode uses playlist gradient */}
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors bg-gradient-to-br ${playlist.color} ${
                          isSelected
                            ? '!bg-green-600 dark:!bg-none'
                            : '!bg-green-100 border border-green-300 group-hover:!bg-green-200 dark:!bg-none dark:border-0'
                        }`}>
                          <PlaylistIcon type={playlist.icon} isPlaying={isSelected} isSelected={isSelected} />
                        </div>
