...
import { useAudio, AMBIENT_SOUNDS, type AmbientSoundType } from '@/contexts/AudioContext';
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-gradient-to-r from-sand-50 to-blue-50 dark:from-sand-950/30 dark:to-blue-950/30">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-sm font-bold text-secondary">TribeWell Music</h2>
            <p className="text-[10px] text-content-muted">Study sounds & playlists</p>
          </div>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="px-4 py-2 border-t border-border bg-surface-muted/30 flex items-center justify-between">
        <p className="text-[10px] text-content-muted">
          {activeAmbientSound ? 'Playing - persists across pages' : activeTab === 'spotify' ? 'Spotify Integration' : 'Select a sound'}
        </p>
        <p className="text-[10px] text-content-muted">
          <kbd className="px-1 py-0.5 bg-surface border border-border rounded font-mono text-[9px]">S</kbd> toggle
        </p>
      </div>
    </aside>
  );
