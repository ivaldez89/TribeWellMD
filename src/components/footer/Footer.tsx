...
// Social links data structure for maintainability
const socialLinks = [
  {
    name: 'Facebook',
    href: 'https://facebook.com/tribewellmd',
...
export interface FooterProps {
  /**
   * Footer variant for different contexts
   * - 'default': Standard footer with explicit colors (authenticated pages)
   * - 'landing': Uses bg-landing-header class (landing/public pages)
   */
  variant?: 'default' | 'landing';
  /**
   * Whether to include a spacer div after the footer
   * Useful for layouts that need content spacing from fixed footer
   */
...
export function Footer({ variant = 'default', includeSpacer = false }: FooterProps) {
  ...
  return (
    <>
      <footer className={`fixed bottom-0 left-0 right-0 z-40 h-12 ${bgClass} text-white shadow-lg`}>
            ...
            <div className="flex items-center gap-1">
              {socialLinks.map((social) => (
                <a
                  ...
                  className={`${social.hiddenOnMobile ? 'hidden sm:flex' : 'flex'} w-6 h-6 rounded-lg bg-white/10 hover:bg-white/20 items-center justify-center transition-colors text-white/70 hover:text-white`}
