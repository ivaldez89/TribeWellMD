...
import type { Flashcard, Rating, ReviewSession, ReviewRecord, DeckFilter, TopicPerformance, MedicalSystem, CardState } from '@/types';
...
import { scheduleCard, getDueCards, calculateStats, previewSchedule } from '@/lib/spaced-repetition/fsrs';

interface UseFlashcardsSupabaseReturn {
  // State
  cards: Flashcard[];
  dueCards: Flashcard[];
...
const FILTERS_STORAGE_KEY = 'step2_filters';

export function useFlashcardsSupabase(): UseFlashcardsSupabaseReturn {
  ...
  // Check authentication and load cards
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        if (user) {
          ...
          // Calculate due cards using FSRS
          const due = getDueCards(fetchedCards);
          setDueCards(due);
        } else {
          setIsAuthenticated(false);
          // Not authenticated - cards will be empty
          setCards([]);
          setDueCards([]);
        }
  ...
  const topicPerformance: TopicPerformance[] = (() => {
    const topicMap = new Map<string, { system: MedicalSystem; cards: Flashcard[] }>();
  ...
  const rateCard = useCallback(async (rating: Rating) => {
    if (!currentCard || !isAuthenticated) return;
    ...
    try {
      ...
      // Create updated spaced repetition data
      const newSRData = {
        state: result.state,
      ...
      // Update in Supabase
      await updateFlashcardSR(currentCard.id, newSRData);

      // Record the review if session is active
      ...
      if (session) {
        const updatedSession: ReviewSession = {
          ...
          reviews: [...session.reviews, {
            ...
            timeSpent: reviewStartTime ? Date.now() - reviewStartTime : 0,
            previousState,
  ...
  // Session management
  const startSessionFn = useCallback(async () => {
    try {
      setError(null);
      ...
      // Still create local session even if Supabase fails
      const newSession: ReviewSession = {
        id: crypto.randomUUID(),
        startedAt: new Date().toISOString(),
        cardsReviewed: 0,
  ...
  const addCard = useCallback(async (
    ...
    if (!isAuthenticated) {
      setError('Must be authenticated to add cards');
      return;
    }
  ...
  const addCards = useCallback(async (newCards: Flashcard[]) => {
    ...
    try {
      ...
      console.log(`Added ${createdCards.length} new cards`);
    } catch (err) {
  ...
  const updateCardFn = useCallback(async (updatedCard: Flashcard) => {
    if (!isAuthenticated) {
      setError('Must be authenticated to update cards');
      return;
    }

    try {
      setError(null);
      const updated = await updateFlashcardApi(updatedCard.id, updatedCard);
  ...
  const deleteCardFn = useCallback(async (id: string) => {
    if (!isAuthenticated) {
      setError('Must be authenticated to delete cards');
      return;
    }
