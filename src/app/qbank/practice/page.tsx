...
function QBankPracticeContent() {
  ...
  const handleCreateFlashcard = useCallback((pearl: string) => {
    ...
    try {
      ...
      // Create a proper Flashcard object
      const newCard: Flashcard = {
        id: crypto.randomUUID(),
        schemaVersion: '1.0',
        createdAt: now,
        updatedAt: now,
        userId: 'local', // Will be updated if user is authenticated
        content: {
      ...
      // Save to localStorage with existing cards
      const updatedCards = [...existingCards, newCard];
      saveFlashcards(updatedCards);

      // Update saved pearl IDs for visual feedback
  ...
  // Header center content: Question # and Timer
  const headerCenter = (
    <>
      {/* Question Number */}
      <div className="flex items-center gap-1.5 px-2 py-1 bg-white/10 rounded-lg">
        ...
        <span className="font-bold text-sm text-white tabular-nums">
          {currentIndex + 1}<span className="text-white/60 font-normal">/{filteredQuestions.length}</span>
        </span>
      </div>
      ...
      {isTimed && (
        <div className="flex items-center gap-1.5 px-2 py-1 bg-white/10 rounded-lg">
          ...
          <span className={`font-mono text-sm font-bold tabular-nums ${timeRemaining <= 10 ? 'text-red-300' : 'text-white'}`}>
  ...
  // Header right content: Mobile question navigator grid
  const headerRight = (
      ...
      <div className="flex items-center justify-between h-full">
        ...
        <div className="flex items-center gap-4 text-xs">
          <span className="hidden sm:inline-flex items-center gap-1.5 text-white/80">
            <kbd className="px-1.5 py-0.5 bg-white/20 rounded font-mono text-white">A-E</kbd>
            <span>select</span>
          </span>
          <span className="hidden sm:inline-flex items-center gap-1.5 text-white/80">
            <kbd className="px-1.5 py-0.5 bg-white/20 rounded font-mono text-white">Enter</kbd>
            <span>submit</span>
          </span>
        ...
        <div className="flex items-center gap-3 text-sm">
          ...
          <span className="font-semibold">{answeredCount}/{filteredQuestions.length}</span>
          {answeredCount > 0 && (
  ...
  return (
    ...
    >
      {/* Left Sidebar - Persistent Question Navigation - constrained between header and footer */}
      ...
      {/* pb-16 ensures content doesn't hide behind fixed footer */}
